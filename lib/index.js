...
exports['default'] = function (_ref) {
    ...
    function checkAndLoadGlobalMap(dir) {
        if (globalMap) return;
        globalMapFile = _path2['default'].resolve(dir, 'langpack.map');
        if (_fs2['default'].existsSync(globalMapFile)) {
            _fs2['default'].readFile(globalMapFile, function (err, data) {
                if (!err) {
                    try {
                        globalMap = JSON.parse(data);
                    } catch (e) {}
                }
                if (!globalMap) globalMap = { indexCount: 0 };
            });
        } else {
            console.log('cannot found: ' + globalMapFile);
            _fs2['default'].closeSync(_fs2['default'].openSync(globalMapFile, 'w'));
            globalMap = { indexCount: 0 };
        }
    }

    function updateGlobalMap() {
        _fs2['default'].writeFile(globalMapFile, JSON.stringify(globalMap, null, 2));
    }
        ...
        visitor: {
            CallExpression: function CallExpression(node, parent, scope, file) {
                ...
                if (node.callee.name == this.context.state.langPackFn) {
                    ...
                    if (!globalMap[sourceFileName]) globalMap[sourceFileName] = globalMap.indexCount++;
